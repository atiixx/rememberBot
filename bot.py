...
async def add_new(ctx, date_text, time_text, *texte):
    ...
    try:
        ...
        if (day > 31 or day < 1 or month > 12 or month < 1 or year > 2030 or year < datetime.datetime.now().year):
            await bot.get_channel(918127437068533770).send("Ungültige Datumsangabe")
            exit

        if (hour < 0 or hour > 24 or minute < 0 or minute > 59):
            await bot.get_channel(918127437068533770).send("Ungültige Zeitangabe")
            exit
        ...
        await bot.get_channel(918127437068533770).send("Bitte folgende Syntax nutzen: \n!neu <TT-MM-JJJJ> <HH:MM> <Text>")
...
async def show_ass(ctx):
    try:
        ...
        await bot.get_channel(918127437068533770).send("Alles erledigt! Keine Assignements übrig :)")
...
async def delete_ass(ctx, index):
    ...
    try:
        ...
        await bot.get_channel(918127437068533770).send("Assignement nicht vorhanden")
